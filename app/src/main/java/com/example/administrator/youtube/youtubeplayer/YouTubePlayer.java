...
public class YouTubePlayer extends WebView {
    ...
    private YouTubePlayerBridge youTubePlayerBridge;
    ...
    protected void initialize(@Nullable YouTubeListener youTubeListener) {
        ...
        youTubePlayerBridge = new YouTubePlayerBridge(this);
        WebSettings settings = this.getSettings();
        settings.setJavaScriptEnabled(true);
        settings.setCacheMode(WebSettings.LOAD_NO_CACHE);
        settings.setMediaPlaybackRequiresUserGesture(false);
        this.addJavascriptInterface(youTubePlayerBridge, "YouTubePlayerBridge");
    ...
    protected void setPlaybackQuality(final String quality) {
        mainThreadHandler.post(new Runnable() {
            @Override
            public void run() {
                Log.e("qualit","change: " + quality);
               // loadUrl("javascript:setPlaybackQualityChanges(\"" + quality + "\");");
                loadUrl("javascript:getPlay();");
            }
        });
    }

    protected void loadVideoWithQuality(final String videoId, final float startSeconds, final String quality) {
        mainThreadHandler.post(new Runnable() {
            @Override
            public void run() {
                loadUrl("javascript:loadVideoWithQuality('" + videoId + "', " + startSeconds + ",'" + quality + "')");
                loadUrl("javascript:getPlay();");
            }
        });
    }
 protected void getQuality(){
     mainThreadHandler.post(new Runnable() {
         @Override
         public void run() {
             loadUrl("javascript:getPlay();");
    ...
    protected void prevVideo() {
        mainThreadHandler.post(new Runnable() {
            @Override
            public void run() {
                loadUrl("javascript:prevVideo()");
            }
        });

    }

    protected void onPlaybackQualityChange(final String quality) {
        mainThreadHandler.post(new Runnable() {
            @Override
            public void run() {
